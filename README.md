...
```public``` 		accessible à **l'intérieur** mais aussi à **l'extérieur** de la classe , niveau bas de sécurité

```private``` 	 accessible à **l'intérieur** de la classe **seulement** , niveau haut de sécurité
...
//Appelle de la méthode de classe regeneration sur l'objet $perso1  Personnage::regeneration($perso1);
//Heroe a maintenant 95 de vie.
```
...
Pour concevoir une classe qui hérite il suffit d'utiliser le mot clef **extends** après le nom de la classe .

class nomDeLaClasseQuiHérite **extends** ClassQuiFaitHériter {}

Nous allons créer une nouvelle classe monstre qui va hériter de la classe Personnage :

```php
class Monstre extends Personnage{		
}
//créons un nouveau monstre
$monstre1 = new Monstre("petit_monstre",20);
//faisons le attaquer perso1
$monstre1->attaque($perso1);


```
Avec un **var_dump** sur le monstre1 issu de la classe Monstre nous voyons qu'il récupère toutes les propriétés et méthodes.
Heroe passe bien de 95 à 75 de vie .

<img src="http://img15.hostingpics.net/pics/670550Capturedecran20161017a231652.png" border="0" alt="Hebergeur d'image" />

Si l'on souhaite, nous pouvons redéfinir certaines propriétés et méthodes et même le constructeur.
